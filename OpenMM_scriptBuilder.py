...
def openMMbuilder(input_coor='input.pdb', force_field='amber99sbildn', water_model='tip3p', platform='CUDA', precision='mixed', device_index=-1,
 		OpenCL_platform_index=-1, nonbonded_method='PME', ewald_error_tolerance=0.0005, constraints='HBonds', constraint_error_tol=0.00001, 
		 rigid_water=True,  nonbonded_cutoff=1.0, random_init_vels=True, generation_temp=300,
		 integrator='Langevin', time_step=2.0, error_tolerance=0.0001, collision_rate=1.0, temperature=300, barostat='None', pressure=1, barostat_interval=25, 
		 state_dataT=True, dcdT=True, pdbT=False, report_interval=1000, equilibration_steps=100, production_steps=1000, minimize=True, 
		 max_minimize_steps=-1, state_data_options=[True,False,True,True,True,False,False,True,False,False]):
	...
	if nonbonded_method == 'PME' or nonbonded_method == 'Ewald':
		code += ('system = forcefield.createSystem(pdb.topology, nonbondedMethod=app.' + nonbonded_method + ', \n    ' + 
			'nonbondedCutoff=' + str(nonbonded_cutoff) + '*unit.nanometers, ' + constraints_tmp + 'rigidWater=' + str(rigid_water) + ', \n    ' + 
	...
	if integrator=='Langevin':
		code += ('integrator = mm.LangevinIntegrator(' + str(temperature) + '*unit.kelvin, ' + str(collision_rate) + '/unit.picoseconds, \n    ' + 
		...
		code += ('integrator = mm.BrownianIntegrator(' + str(temperature) + '*unit.kelvin, ' + str(collision_rate) + '/unit.picoseconds, \n    ' + 
			str(time_step) + '*unit.femtoseconds)\n')
	...
	if (barostat != 'None'):
		code += ('system.addForce(mm.MonteCarloBarostat(' + str(pressure) + '*unit.atmospheres, ' + str(temperature) + '*unit.kelvin, ' + str(barostat_interval) + '))\n')
	...
	if platform == 'CUDA' or platform == 'OpenCL':
		...
		if platform == 'CUDA':
			if device_index == -1:
				...
				code += ('properties = {\'CudaPrecision\': \'' + precision + '\', \'CudaDeviceIndex\': \'' + str(device_index) + '\'}\n')
			...
			if device_index == -1 and OpenCL_platform_index == -1:
				...
				if device_index == -1:
					code += ('properties = {\'OpenCLPrecision\': \'' + precision + '\', \'OpenCLPlatformIndex\': \'' + str(OpenCL_platform_index) + '\'}\n')
				elif OpenCL_platform_index == -1:
					code += ('properties = {\'OpenCLPrecision\': \'' + precision + '\', \'OpenCLDeviceIndex\': \'' + str(device_index) + '\'}\n')
				else:
					code += ('properties = {\'OpenCLPrecision\': \'' + precision + '\', \'OpenCLPlatformIndex\': \'' + str(OpenCL_platform_index) + '\', \n              '
	...
	if state_dataT:
		code += ('simulation.reporters.append(app.StateDataReporter(stdout, ' + str(report_interval) + ', step=' + str(str(state_data_options[0])) + ', \n' +
    		'time=' + str(state_data_options[1]) + ', potentialEnergy=' + str(state_data_options[4]) + ', kineticEnergy=' + str(state_data_options[5]) + 
			...
			+ ', speed=' + str(state_data_options[2]) + ', totalSteps=' + str(production_steps) + ', separator=\'\\t\'))\n')
...
if __name__ == '__main__':
	openMMbuilder(state_dataT=True, pdbT=True, dcdT=False, report_interval=11, equilibration_steps=10001, production_steps=5000, minimize=True, max_minimize_steps=3)
"""